...
async function findCandidateJournals(searchText: string): Promise<Journal[]> {
  ...
  const response = await fetch(url, {
    headers: { "User-Agent": "MabrigPublishAI/0.3 (publisher technical services engine)" },
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const ai = await generateReport({ clientName, title, field, articleType, abstract, manuscriptText, targetJournal, services, candidates });
    const report = ai?.text || fallbackReport({ title, field, articleType, abstract, targetJournal, services });
